...
Ext.define('Ext.ux.grid.FiltersFeature', {
    ...
    createFilters: function() {
        ...
        Ext.Array.each(grid.down('headercontainer').getGridColumns(), function (column) {
    ...
    initParaData: function(){
	   	 var items = this.getFilterItems();
	     var clsname=this.view.getStore().proxy.className;
	     if(items!=null && items.length>0){
	    	 var clas=items[0].grid.classAttributes;
	    	 for(var i=0;i<clas.length;i++){
	    		 var tp=clas[i].type;
	    		 if(tp=="LOOKUP"){
	    			 _CMCache.getLookupStore(clas[i].lookup);
	    		 }else if(tp=="REFERENCE"){
	    			 CMDBuild.Cache.getReferenceStore(clas[i]);
	    		 }
	    	 }
	     }
    },
    ...
    reload : function () {
        ...
        store.proxy.extraParams.filterArry=this.initParaStr(store);
        if (me.local) {
    ...
    getLookUpName :function(item){
    	 var name="";
    	 var clas=item.grid.classAttributes;
    	 for(var i=0;i<clas.length;i++){
    		 var tp=clas[i].type;
    		 if(tp=="LOOKUP" && item.dataIndex==clas[i].name){
    			 return clas[i].lookup;
    		 }
    	 }
    	 return name;
    },
    
    initParaStr : function (sr) {
    	 var filterStr="";
    	 var clsname=sr.proxy.className;
    	 var arrayObj=new Array();
    	 var arrayOperator=new Array();
    	 var items = this.getFilterItems();
    	 for(var i=0;i<items.length;i++){
    		 var tempName=items[i].dataIndex;
    		 var tempValue=items[i].inputItem.value;
    		 if(items[i].active && tempName!=null && tempValue!='undefined' && tempValue!=null && tempValue!=""){
    			 var type=this.getFilterTypeOperator(items[i].grid.classAttributes,tempName);
    			 if(type=="STRING"){
    				 arrayOperator[tempName]="contain";
    				 arrayObj[tempName]=tempValue;
    			 }else if(type=="LOOKUP"){
    				 var ste = _CMCache.getLookupStore(this.getLookUpName(items[i]));
    				 var vl=this.getFilterLookUpValue(ste.data.items,tempValue);
    				 if(vl!=null && vl!=""){
    					 arrayObj[tempName]=vl;
        				 arrayOperator[tempName]="equal";
    				 }
    			 }else{
    				var vl=this.getFilterRefValue(items[i].grid.classAttributes,tempName,tempValue);
    				if(vl!=null && vl!=""){
   					 	arrayObj[tempName]=vl;
   					 	arrayOperator[tempName]="equal";
   				 	}
         ...
         var allFilter="";
         var count =0;
         for(var n in arrayObj){
        	 count++;
           	var temp="{'simple':{'value':['"+arrayObj[n]+"'],'parameterType':'fixed'"+","+"'attribute':'"+n+"','operator':'"+arrayOperator[n]+"'}}";
           	if(allFilter==""){
           		allFilter=temp;
           	}else{
           		allFilter=allFilter+","+temp;
           	}
         }
         if(count==0){
        	 return "";
         ...
         if(count>1){
        	 filterStr="{'attribute':{'and':["+allFilter+"]}}";
         }else{
           filterStr="{'attribute':"+allFilter+"}";
        }
    	return encodeURIComponent(filterStr);
    },
    
    getFilterRefValue : function (attrs,tempName,tempValue){
    	var classAtr;
    	var vl="";
    	for(var i=0;i<attrs.length;i++){
    		if(attrs[i].name==tempName){
    			classAtr=attrs[i];
    			break;
    		}
    	}
    	var sto = CMDBuild.Cache.getReferenceStore(classAtr);
    	if(sto!=null){
    		var items=sto.data.items;
    		for(var j=0;j<items.length;j++){
        		if(items[j].data.Description.toLowerCase()==tempValue.toLowerCase()){
        			vl=items[j].data.Id;
        			break;
        		}
        	}
    ...
    getFilterLookUpValue : function (values,tempVl) {
    	var vl="";	
    	for(var i=0;i<values.length;i++){
    		if(values[i].data.Description.toLowerCase()==tempVl.toLowerCase()){
    			return values[i].data.Id;
    		}
    	}
    	return vl;
    },
    
    getFilterTypeOperator : function (classAttributes,tempName) {
    	var type="STRING";	
    	for(var i=0;i<classAttributes.length;i++){
    		if(classAttributes[i].name==tempName){
    			return classAttributes[i].type;
    		}
    	}
    	return type;
    },
    ...
    addFilter : function (config) {
        var me = this,
            columns = me.getGridPanel().down('headercontainer').getGridColumns(),
