...
import org.cmdbuild.servlets.json.management.dataimport.xls.XLSImporter;
...
import org.cmdbuild.servlets.json.management.export.xls.XlsExporter;
...
public class DefaultDataAccessLogic implements DataAccessLogic {
	...
	private static final String DESCRIPTION_ATTRIBUTE = org.cmdbuild.dao.driver.postgres.Const.DESCRIPTION_ATTRIBUTE;
	...
	@Override
	public Card fetchCardByDescription(final String className, final String description) {
		CMCard cm=fetchCMCardByDescription(className, description);
		if(cm!=null){
			return from(asList(cm)) //
			.transform(CMCARD_TO_CARD) //
			.first() //
			.get();
		}else{
	...
	@Override
	public Map<String,String> getCardMapData(final String className, final String description) {
		Map<String,String> oldMap=new HashMap<String, String>();
		final CMClass entryType = dataView.findClass(className);
		final CMQueryResult result = dataView.select(anyAttribute(entryType)) //
				.from(entryType) //
				.where(condition(attribute(entryType, DESCRIPTION_ATTRIBUTE), eq(description))) //
				.limit(1) //
				.run(); //
		...
		List<CMCard> cards=asList(row.getCard(entryType));
		if(cards==null||cards.isEmpty()){
			return null;
		}
		CMCard card=cards.get(0);
		Iterable<Entry<String, Object>> it=card.getValues();
		for(Entry<String, Object> entity:it){
	...
	@Override
	public CMCard fetchCMCardByDescription(final String className, final String description) {
		final CMClass entryType = dataView.findClass(className);
		return fetchCMCardByDescription(entryType, description);
	}

	private CMCard fetchCMCardByDescription(final CMClass entryType, final String description) {
		try {
			final CMQueryResult result = dataView.select(anyAttribute(entryType)) //
					.from(entryType) //
					.where(condition(attribute(entryType, DESCRIPTION_ATTRIBUTE), eq(description))) //
	...
	@Override
	public void updateCardBatch(final Card userGivenCard) {
	...
	@Override
	public File exportClassAsXlsFile(final String className) {
		final CMClass fetchedClass = dataView.findClass(className);
		final String fileName = fetchedClass.getIdentifier().getLocalName() + ".xls";
		final String dirName = System.getProperty("java.io.tmpdir");
		final File targetFile = new File(dirName, fileName);
		final DataExporter dataExporter = new XlsExporter(targetFile,className);
		final CMDataSource dataSource = new DBDataSource(dataView, fetchedClass);
		return dataExporter.export(dataSource);
	}
	
	@Override
	public XLSData importXlsFileFor(final FileItem xlsFile, final Long classId)
			throws IOException, JSONException {
		final CMClass destinationClassForImport = dataView.findClass(classId);
		final XLSImporter xlsImporter = new XLSImporter( //
				dataView, //
				lookupStore, //
				destinationClassForImport
		);

		final XLSData xlsData = xlsImporter.getXlsDataFrom(xlsFile);
		return xlsData;
	}
