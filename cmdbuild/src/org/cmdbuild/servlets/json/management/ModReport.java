...
public class ModReport extends JSONBaseWithSpringContext {
	...
	public DataHandler printReportFactory( //
		...
		if(reportFactory.getReportExtension().toString().equals("XLS")){
			int count=0;
			if(reportFactory instanceof ReportFactoryTemplateList){
				count=((ReportFactoryTemplateList)reportFactory).getAttributeNamesSorted().size();
			}
			reportFactory.sendReportToStreamWithXLS(outputStream,count);
		}else{
