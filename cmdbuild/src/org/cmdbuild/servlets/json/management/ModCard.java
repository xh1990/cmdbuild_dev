...
import static org.cmdbuild.servlets.json.CommunicationConstants.FILTER_ARRY;
...
public class ModCard extends JSONBaseWithSpringContext {
	...
	public JSONObject getCardList( //
			...
			@Parameter(value = FILTER_ARRY, required = false) final String filterArry,
			@Parameter(value = FILTER, required = false)  JSONObject filter, //
		...
		if(filterArry!=null&&!filterArry.equals("")&&(filter==null || filter.length()<=0)){
			try{
				filter=new JSONObject(URLDecoder.decode(filterArry,"utf-8"));
			}catch(Exception e){
				e.printStackTrace();
			}
		}
